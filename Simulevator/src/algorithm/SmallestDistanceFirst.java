 ...
 * 
 */
public class SmallestDistanceFirst extends Algorithm {


	public SmallestDistanceFirst(String algorithm_name) {
	...
	synchronized public int onEventExternalRequest(ArrayList<ElevatorController> elevators,
		...
		for (int i = 0; i < elevators.size(); i++) {
			if (elevators.get(i).getDistancedTravelled() < minDist && elevators.get(i).isActive()) {
				minDist = elevators.get(i).getDistancedTravelled();
				chosenOne = i;
			}
		}
