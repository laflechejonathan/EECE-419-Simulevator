...
public class ClosestFirst extends Algorithm {
	...
	public ClosestFirst(String algorithm_name ) {
	...
	synchronized public int onEventExternalRequest(ArrayList<ElevatorController> elevators,
		...
		double closest = 1000;
		for (int i = 0; i < elevators.size(); i++) {
			if (Math.abs(elevators.get(i).getFloor()- extRequest.requestFloor) < closest && elevators.get(i).isActive())
			{
				closest = Math.abs((elevators.get(i).getFloor() - extRequest.requestFloor));
				chosenOne = i;
			}
		}
